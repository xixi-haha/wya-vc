<template>
	<div class="vcm-picker">
		<c-popup v-model="show" @change="handleChange">
			<div v-if="showHeader" class="__header">
				<div class="__item __left" v-if="cancelText" @click="handleCancel">{{ cancelText }}</div>
				<div class="__item __title">{{ title }}</div>
				<div class="__item __right" v-if="confirmText" @click="handleOk">{{ confirmText }}</div>
			</div>
			<div class="__main">
				<c-col
...
export default {
	...
	computed: {
		showHeader() {
			return this.cancelText || this.confirmText || this.title;
		}
	},
	mounted() {
		this.$nextTick(() => {
