<template>
	<div>
		<c-popup v-model="show" :show-header="showToolbar" :loading="true" @popup-close="handlePickerClose">
			...
			<c-col v-for="(item,index) in slots" :ref="`colVal${index}`" :key="index" :index="index"
:divider="item.divider" :values="item.values" :content="item.content" :flex="item.flex" :text-align="item.textAlign" :class-name="item.className" :value-key="valueKey" @change="handleColChange" />
		</c-popup>
	</div>
</template>
